...
app.controller('manual_booking', ['$scope', '$http', '$compile', '$filter', function($scope, $http, $compile, $filter) {
	...
	{
  		pickup_location_autocomplete = new google.maps.places.Autocomplete(document.getElementById('input_pickup_location'));
  	  	pickup_location_autocomplete.addListener('place_changed', pickup_location_address);

  	  	drop_location_autocomplete = new google.maps.places.Autocomplete(document.getElementById('input_drop_location'));
	...
	function getVehicleTypes(id=0) {
		...
		date_time=$('#input_date_time').val()
      	$http.post(REQUEST_URL+'/search_cars', {
        	...
        	if (response.status_code==0 || response.code =="ZERO_RESULTS") {
        		$('.error_msg').hide();
        		$('.error_msg').html('');
	...
	function fare_detail(){
		// $scope.fare_currency = $scope.vehicle_detail.currency
