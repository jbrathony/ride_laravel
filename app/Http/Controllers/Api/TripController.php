...
class TripController extends Controller
{
	...
	protected function getTripDetails($trip_id,$user)
	{
		...
		if($trip->status == 'Scheduled') {
			$get_min_time = $this->request_helper->GetDrivingDistance($trip->pickup_latitude, $driver_location->latitude, $trip->pickup_longitude, $driver_location->longitude);
		}
		else {
			$get_min_time = $this->request_helper->GetDrivingDistance($driver_location->latitude, $trip->drop_latitude, $driver_location->longitude, $trip->drop_longitude);
		}
		...
		if ($get_min_time['status'] != "success") {
			...
			$arrival_time = round(floor(round($get_min_time['time'] / 60)));
		}
		...
		return $trips->map(function($trip) use ($user_type) {
			if(isset($trip->booking_type)) {
				$trip_data = array(
					...
					'total_fare'	 	=> $trip->fare_estimation,
					'status'		 	=> $trip->status,
				);
	...
	public function get_upcoming_trips(Request $request)
	{
        ...
        $schedule = ScheduleRide::where('user_id', $user_details->id)->where('status','Pending')->orderBy('id','DESC')->paginate($this->paginate_limit)->toJson();
