...
class LiveRestrict
{
    ...
    {
        if (isLiveEnv()) {
            if (in_array(request()->segment(1),['admin','company'])){ // restriction for for admin & company urls
                $url = url()->current();
                ...
                if (($_POST||$delete_url) && !in_array($admin_url[1],$admin_unrestricted_url) ) {
                    Session::flash('alert-class', 'alert-error');
                    Session::flash('message', 'Data add,edit & delete opration are restricted in live.');
